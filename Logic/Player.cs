...
public class Player : MonoBehaviour, ICharacter
{
    ...
    // 1st overload - by ID
    public void PlayACreatureFromHand(int UniqueID, int tablePos, TablePosition table)
    {
        PlayACreatureFromHand(CardLogic.CardsCreatedThisGame[UniqueID], tablePos, table);
    }

    // 2nd overload - by logic units
    public void PlayACreatureFromHand(CardLogic playedCard, int tablePos, TablePosition table)
    {
        // Debug.Log(ManaLeft);
        ...
        tables[(int)table].CreaturesOnTable.Insert(tablePos, newCreature);
        // 
        new PlayACreatureCommand(playedCard, this, tablePos, newCreature.UniqueCreatureID,table).AddToQueue();
