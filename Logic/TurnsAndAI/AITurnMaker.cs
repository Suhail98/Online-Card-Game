        ...
        foreach (CardLogic c in p.hand.CardsInHand)
        {
            if (c.CanBePlayed)
            {
                ...
                else
                {
                    ...
                    // TODO fix this haaaaaaaa
                    p.PlayACreatureFromHand(c, 0,0);
                    InsertDelay(1.5f);
                    return true;
                }
    ...
    bool AttackWithACreature()
    {
        ...
        foreach (CreatureLogic cl in table.CreaturesOnTable)
        {
            if (cl.AttacksLeftThisTurn > 0)
            {
                ...
                // TODO fix this haaaaaaaa
                if (p.otherPlayer.tables[0].CreaturesOnTable.Count > 0)
                {
                    int index = Random.Range(0, p.otherPlayer.tables[0].CreaturesOnTable.Count);
                    CreatureLogic targetCreature = p.otherPlayer.tables[0].CreaturesOnTable[index];
